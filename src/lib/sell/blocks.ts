import type { SellBlockDef, Surface } from "./types";

const BOTH: Surface[] = ["club", "landing"];
const CLUB: Surface[] = ["club"];
...
export const SELL_BLOCK_DEFS: SellBlockDef[] = [
  ...
  },
  {
    ...
    surfaces: CLUB, duplicable: false,
    fields: [heading, body],
  ...
  },
  {
    ...
    surfaces: LANDING, duplicable: true,
    fields: [heading, body],
  ...
  },
  {
    ...
    surfaces: LANDING, duplicable: false,
...
export const SELL_BLOCK_MAP: Record<string, SellBlockDef> = Object.fromEntries(SELL_BLOCK_DEFS.map(b => [b.type, b]));

export function sellBlocksForSurface(surface: Surface): SellBlockDef[] {
  return SELL_BLOCK_DEFS.filter(b => b.surfaces.includes(surface));
}

export function sellDefForType(type: string): SellBlockDef | undefined {
  return SELL_BLOCK_MAP[type];
}

/** The default, simple Club page — Level 1. */
...
/** A sensible starting landing page — Level 2. */
